...
| --- | --- | --- | --- |
| `POST /api/vouchers/claim` | `apiRequest` body `{ dropId, deviceId }` (optional email/phone in schema, not always sent) | `POST /api/v1/vouchers/claim` + `DeviceGuard`; `ClaimVoucherDto` requires **dropId + deviceId** (regex `[a-zA-Z0-9_-]+`), optional `hunterId` | **Partial:** No `userEmail`/`userPhone` on Nest DTO. **Response:** Contract `{ voucher, drop, magicLink, promoCode }`; Nest returns **`VoucherResponseDto`** (nested `drop`, `magicToken`, etc.) — **different shape**. |
...
| `POST /api/hunter/login` | email, password, deviceId | `POST /api/v1/auth/hunter/login` — **`LoginDto`** email + password **only** (no deviceId in DTO) | **deviceId** handling differs. |
