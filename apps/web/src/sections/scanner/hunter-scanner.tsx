...
export default function HunterScannerPage() {
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      const html5QrCode = new Html5Qrcode("qr-reader-hunter");
  ...
  const redeemerMerchantId = (
    profile as { redeemerMerchantId?: string | null } | null | undefined
  )?.redeemerMerchantId;
  const canRedeem = Boolean(
    redeemerMerchantId && redeemerMerchantId.length > 0
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        ) : profile?.email && !canRedeem ? (
          <Card className="border-amber-500/30 bg-amber-500/5">
            <CardContent className="pt-6 text-center space-y-3">
              <ShieldAlert className="w-12 h-12 text-amber-600 mx-auto" />
              <h2 className="text-lg font-semibold">
                {t("hunterScan.notLinkedTitle")}
              </h2>
              <p className="text-sm text-muted-foreground">
                {t("hunterScan.notLinkedBody")}
              </p>
              <Button asChild variant="outline" className="mt-2">
                <Link href="/profile">{t("nav.backToProfile")}</Link>
              </Button>
            </CardContent>
          </Card>
        ) : canRedeem && scanResult ? (
          ...
          <div className="space-y-6">
            <Card className="overflow-hidden">
              ...
              <CardContent>
                ...
                >
                  <div id="qr-reader-hunter" className="w-full h-full" />
